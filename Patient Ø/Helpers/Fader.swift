...
//  Patient Ø
//
//  Created by David Jupijn on 21/06/16.
...
let Fader_defaultFadeDurationSeconds = 3.0
let Fader_defaultVelocity = 2.0
...
public class Fader: NSObject {
  ...
  private var fadeDurationSeconds = Fader_defaultFadeDurationSeconds
  private var fadeVelocity = Fader_defaultVelocity
  ...
  func fadeIn(duration: Double = Fader_defaultFadeDurationSeconds,
              velocity: Double = Fader_defaultVelocity, onFinished: ((Bool)->())? = nil) {
  ...
  func fadeOut(duration: Double = Fader_defaultFadeDurationSeconds,
               velocity: Double = Fader_defaultVelocity, onFinished: ((Bool)->())? = nil) {
  ...
  func fade(fromVolume fromVolume: Double, toVolume: Double,
                       duration: Double = Fader_defaultFadeDurationSeconds,
                       velocity: Double = Fader_defaultVelocity, onFinished: ((Bool)->())? = nil) {
  ...
  private func startTimer() {
    ...
    timer = NSTimer.scheduledTimerWithTimeInterval(1 / volumeAlterationsPerSecond, target: self,
                                                   selector: #selector(Fader.timerFired(_:)), userInfo: nil, repeats: true)
  }
  ...
  func timerFired(timer: NSTimer) {
    ...
    let currentTimeFrom0To1 = Fader.timeFrom0To1(
    ...
    if fadeIn {
      volumeMultiplier = Fader.fadeInVolumeMultiplier(currentTimeFrom0To1,
                                                                        velocity: fadeVelocity)

      newVolume = fromVolume + (toVolume - fromVolume) * volumeMultiplier

    } else {
      volumeMultiplier = Fader.fadeOutVolumeMultiplier(currentTimeFrom0To1,
                                                                         velocity: fadeVelocity)
